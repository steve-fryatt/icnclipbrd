...
MessageHandlerRAMTransmitLoopExit
        ...
        MOV     R0,#Message_RAMFetch		; ...otherwise send another Message_RAMFetch to get the rest of the data.
        STR     R0,[R1,#16]
...
;        B       WimpMessageBounced
        ...
        SWI     Wimp_SendMessage                ; This should send the Message_RAMFetch, but seems to never be sent???
        B       WimpPollLoop
...
DataLoadPasteLoop
        ...
        MOV     R3,R0				; Insert the byte into the kayboard buffer, prefixing
...
InitialisationCode
	...
	MOV	R0,#&FF00			; Initialise the flags (enable all keypresses)
...
PostFilterPasteReplaceInsert
        ...
        CMP     R2,#0				; to insert the text into the kayboard buffer until a
        LDMEQFD R13!,{R0-R4,PC}			; zero byte is found.

        TST     R2,#&80				; Handle the zero-byte prefix on chars >127 for OS_Byte &8A.
...
RemoveExtUpdateLoop
        ...
        BEQ     PostFilterRefreshIcon
						...
						; Capitalise the first letter of each alphabetic block.
        CMP     R1,#&41                         ; 'A'
        MOVLT   R14,#1
        BLT     RemoveExtUpdateLoop
        CMP     R1,#&7A                         ; 'z'
        BGT     RemoveExtUpdateLoop
        CMP     R1,#&61                         ; 'a'
        BGE     RemoveExtCheckCase
        CMP     R1,#&5A                         ; 'Z'
        BLE     RemoveExtCheckCase
        MOV     R14,#1
        B       RemoveExtUpdateLoop

RemoveExtCheckCase
        CMP     R14,#1
        BICEQ   R1,R1,#&20			; Force the first letter to upper.
        ORRNE   R1,R1,#&20                      ; Force subsequent letters to lower.
        STRB    R1,[R2,#-1]
        MOV     R14,#0
        B       RemoveExtUpdateLoop
...
CommandKeys_Set
	...
	AND	R2,R2,#&FF			; Mask the number down to a single byte, to be safe.

	LDR	R0,[R12,#WS_FlagWord]		; Take the number and replace the 2nd byte of the flags with it.
	BIC	R0,R0,#&FF00
	ORR	R0,R0,R2,ASL #8
	STR	R0,[R12,#WS_FlagWord]
...
HelpString
        DCB     "IcnClipBrd\t0.11 (18 Jul 2007) 32-bit © Thomas Leonard & Stephen Fryatt",0
...
CommandKeysHelp
	...
	DCB	"<new keys> is a number formed of binary digits %ZXVQKEDC.",13
